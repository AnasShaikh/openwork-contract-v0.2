...
/**
 * @title OpenWorkRewardsContract (Non-Upgradeable)
 * @notice Handles reward calculations for the OpenWork platform
 ...
 */
contract NativeRewardsContract is
    Initializable,
    OwnableUpgradeable
{
    // ==================== REWARD STRUCTURES ====================
    ...
    // NOTE: adminSetTeamTokensClaimed() has been REMOVED for security
    // Team token claims are now managed solely through the normal claim flow

    // ==================== REWARD BANDS INITIALIZATION ====================

    function _initializeRewardBands() private {
        // 20 reward bands, each distributing 30M OW tokens
        // Format: minAmount (USDC 6 decimals), maxAmount, owPerDollar (earning rate, 18 decimals), tokensPerGovernanceAction (unlock rate, 18 decimals)
     ...
     * @param minAmount Minimum cumulative platform amount for this band (USDC 6 decimals)
     * @param maxAmount Maximum cumulative platform amount for this band (USDC 6 decimals)
     * @param owPerDollar OW tokens awarded per USDC for earning (18 decimals)
    ...
    /**
     * @dev Get the current reward band based on total platform payments
     * @notice SECURITY FIX: Uses < instead of <= for maxAmount to prevent boundary issues
     */
    function getCurrentBand() public view returns (uint256) {
        for (uint256 i = 0; i < rewardBands.length; i++) {
            if (totalPlatformPayments >= rewardBands[i].minAmount &&
                totalPlatformPayments < rewardBands[i].maxAmount) {  // FIXED: Changed <= to <
                return i;
            }
        }
        // If at or beyond last band's max, return last band
    ...
    function calculateTokensForRange(uint256 fromAmount, uint256 toAmount) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < rewardBands.length && currentAmount < toAmount; i++) {
            ...
            if (bandStart < bandEnd) {
                ...
                uint256 tokensInBand = (amountInBand * band.owPerDollar) / 1e6; // Convert USDC (6 decimals) to tokens

    ...
    UUPSUpgradeable
{
    ...
    function releasePaymentCrossChain(
        ...
        // Get target chain NOWJC address for minting
        address targetNOWJC = targetChainNOWJC[_targetChainDomain];
        require(targetNOWJC != address(0), "Target chain NOWJC not set");
        ...
        // Note: USDC minted to target chain NOWJC, not end user directly
        // End user can claim from target chain NOWJC later
        emit PaymentReleased(_jobId, _jobGiver, targetNOWJC, _amount, job.currentMilestone);
    }
    ...
    function releaseDisputedFunds(
       ...
       // require(msg.sender == nativeAthena, "Only Native Athena can resolve disputes");
        ...
        emit DisputedFundsReleased("dispute", _recipient, _targetChainDomain, _amount);
    }

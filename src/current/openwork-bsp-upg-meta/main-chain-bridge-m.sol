...
contract ThirdChainBridge is OAppSender, OAppReceiver {
    ...
    function _lzReceive(
        ...
        // ==================== REWARDS CONTRACT MESSAGES ====================
            ...
            IRewardsContract(rewardsContract).handleCreateProfile(user, referrer, _origin.srcEid);
        } else if (keccak256(bytes(functionName)) == keccak256(bytes("updateRewardsOnPayment"))) {
            require(rewardsContract != address(0), "Rewards contract not set");
            (, address jobGiver, address jobTaker, uint256 amount) = abi.decode(_message, (string, address, address, uint256));
            IRewardsContract(rewardsContract).handleUpdateRewardsOnPayment(jobGiver, jobTaker, amount, _origin.srcEid);
        } else if (keccak256(bytes(functionName)) == keccak256(bytes("notifyGovernanceAction"))) {
            require(rewardsContract != address(0), "Rewards contract not set");
            (, address account) = abi.decode(_message, (string, address));
            IRewardsContract(rewardsContract).handleGovernanceActionNotification(account, _origin.srcEid);
            ...
            IRewardsContract(rewardsContract).handleStakeDataUpdate(staker, amount, unlockTime, durationMinutes, isActive, _origin.srcEid);
        }
